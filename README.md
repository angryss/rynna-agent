...
- **Shared development policy:** every core model request includes software-development/terminal guidance, private reasoning with a Reason → Act → Observe loop, and its actual available tool inventory. Configured prompts, project context, and skills are preserved. See [runtime system prompt](docs/system-prompt.md).
...
- [OpenAI Codex CLI](https://developers.openai.com/codex/cli/) to configure OpenAI with a ChatGPT subscription or API key; account-backed chat requires a compatible Codex app-server, not an exact CLI version
...
Context-window sizes are filled and saved automatically for new models and saved models whose size is blank, when the selected provider reports a limit. Existing manual values are preserved. OpenAI account limits come from the selected Codex home’s model metadata; Rynna uses the reported maximum (falling back to the standard window) and passes the saved size into Codex’s session configuration. `Codex default` resolves through the account’s effective model configuration. API catalogs can supply context-window, context-length, serving-length, or maximum-input-token metadata; output-token limits are not treated as context windows. Unknown or custom models remain editable without an invented limit. Saved changes take effect after restart.

OpenAI account models are initially disabled to preserve existing defaults. When enabled, they can use the same permission-controlled Rynna tools, skills, MCP adapters, and delegated subagents as other tool-capable providers. Only tools supplied by the active profile are advertised through Codex app-server dynamic tools; Codex ambient shell, filesystem, web, and other native tool operations remain prohibited. Read-only `host_info` is available by default without a shell. Commands still require an explicit capability in normal mode; explicit YOLO supplies unrestricted native commands through Rynna, never through Codex ambient execution. Incompatible app-server dynamic-tool protocols fail closed rather than falling back to an ambient shell. See [the Codex tool bridge contract](docs/codex-tools.md). Credentials remain profile-scoped settings: newly signed-in accounts use Rynna's private Codex directory, while **reuse existing credentials** uses the ordinary Codex account. `RYNNA_CODEX_PATH` and `RYNNA_CODEX_HOME` apply to the CLI/server as well as desktop. Deleting the profile's OpenAI credential entry prevents subsequent account-model requests even before restart.

The desktop app also exposes **Connect OpenAI**. Choose **Use ChatGPT subscription** to complete Codex's supported browser sign-in, or enter an OpenAI API key for usage-based API billing. When adding a ChatGPT-backed OpenAI provider later, Rynna checks the user's existing Codex account and asks whether to reuse those ChatGPT credentials or complete a new browser sign-in in Rynna's private Codex configuration directory. Rynna verifies reused credentials with `codex login status`, passes API keys to Codex over stdin, and never returns credentials through Tauri IPC. After connecting, select the `openai-account` profile to send prompts through that account. This account-backed profile receives the active profile’s permission-controlled Rynna tools through the dynamic-tool bridge. Its ephemeral Codex thread has no native execution environment; shell, image, planning, and web-search tools are disabled, and unadvertised or native tool lifecycle items abort the response. Rynna does not probe `codex --version` or require an exact CLI release. Compatibility is determined by the app-server interaction: protocol errors, malformed JSON, missing thread/turn IDs, and disabled tool lifecycle items still fail the request. Read-only sandboxing, `approvalPolicy: never`, and bounded response handling remain in place. A future Codex release that changes the required protocol may need a Rynna update; a different version banner alone does not block chat.

## Default read-only tools

Profiles with no explicit filesystem capability receive read-only file inspection, directory listing, file finding, regex search and indexed code search scoped to their selected project (or profile default directory / cwd). `host_info` reports basic OS and architecture information without a shell. Writes and commands are not silently granted; explicit capabilities remain the normal-mode setup path. See the [tool permissions contract](BACKEND-CONTRACT.md) for restrictions, YOLO overrides and retained limits.
...
Delegation is one level deep and sequential; helpers cannot delegate to other helpers. Each helper uses the existing bounded model loop. Parent and helpers share one 64-call budget and one 8 MiB tool-result byte budget per response, and the parent's 300-second aggregate deadline covers delegated work. All built-in provider adapters, including both subscription adapters, support permission-controlled delegation. Custom providers that disable external tools do not expose delegation.
...
`config.yaml`, or update the profile's `yolo` setting through the profile API. Same-name mode edits apply immediately in web and desktop; the default is `false`. A CLI `--yolo` process override remains effective until restart without the flag.

YOLO is an execution-policy bypass, not only a prompt instruction. It supplies native filesystem and arbitrary command tools even with no capabilities, ignores toolset switches and configured tool restrictions, and bypasses native/core/workflow execution budgets without Rynna confirmation prompts. Helpers inherit it. Missing task information may still require clarification.

Kernel permissions, existing process credentials, provider authentication, argument/protocol validation, cancellation and model-context limits remain. No automatic sudo or privilege escalation occurs. See the [full contract and retained protocol bounds](BACKEND-CONTRACT.md) before enabling it.
