...
Profiles with no explicit filesystem capability receive read-only file inspection, directory listing, file finding, regex search and indexed code search scoped to their selected project (or profile default directory / cwd). OS inspection uses `run_command` under the existing command permissions. Writes and commands are not silently granted; explicit capabilities remain the normal-mode setup path. See the [tool permissions contract](BACKEND-CONTRACT.md) for restrictions, YOLO overrides and retained limits.
